...
- **Fork point:** `699edac3` (`v0.5.55`) · **last merged:** `v0.5.69`
...
| `handlers/chatCore/streamingHandler.js` | logs, smartlogs | `+ beside` | Both fields destructured and forwarded in **both** record calls. | A streaming row is upserted twice under one id; a field on only one write disappears on the second. Also owns the placeholder text the outcome logic compares against. Its signature is now the one line both sides append to — `v0.5.69` put `credentials` there for the session-scoped translator features, so a resolution that keeps only one side's additions builds and runs either way. |
...
| `src/sse/services/backgroundTokenRefresh.js` | The token-status module mirrors it twice: its four eligibility conditions, and its lead-time formula. Both operands imported. | **The most consequential file for `tokenstat`.**<br>• Read the connection loader as well as the selector — the active filter lives there, and the fork's first version of the mirror missed it for that reason.<br>• A fifth condition anywhere in the path is the quiet failure: the sweep skips connections the fork still shows a due time for.<br>• **A fifth condition is not the only way the due time drifts.** `v0.5.69` replaced the parallel sweep with a sequential one that sleeps between accounts — twelve seconds plus jitter for the two Google-backed providers, under two for the rest — so with enough due connections one tick now outlasts its own interval. Nothing the fork mirrors changed and nothing fails; the displayed instant just gets coarser, which `Specs.md` 2.2 already admits it is. Worth re-reading whenever the delays or the interval move, because the two together decide how coarse. |
...
| `open-sse/handlers/chatCore.js` 401 block, `open-sse/executors/base.js` | The refresh path `tokenstat` deliberately does not observe — it calls the executor directly, so the result never passes the credential merge and a failure is a log line and nothing else. | If upstream ever routes it through the shared refresh entry point, the gap closes for free. **As of `v0.5.69` it has not:** the retry still calls the executor directly, it only gained the same two session arguments as the first call. Check again each merge — the two `executor.execute` calls are the thing to look at, and an upstream test now counts them. |
...
| `tests/unit/github-monthly-usage-lock.test.js` | One of three upstream tests that constrain fork-edited code, and **the only one expected to fail.** It mocks the db module with two exports, and the test runner throws on reading an undeclared export — so the settings read the `locks` feature added must survive a **synchronous** throw. | Its two cases take different branches.<br>• One never consults settings and passes.<br>• The other reaches the resolver and **fails by design**: it pins upstream's two minutes by reading it through this feature's own fallback. Expect that failure.<br>• Only the test comparison catches a real regression here. |
| `tests/unit/fetch-success-clears-account.test.js` | The second, and it **passes** — keep it that way. It asserts the web-fetch lock key *and the exact argument list* of all three `auth.js` calls, so it is the one upstream test that pins the fork's optional-argument shaping rather than just its behaviour. | • A trailing argument added unconditionally to selection or marking fails it on arity alone, with the behaviour still correct — which reads as a broken test rather than a broken call.<br>• It is also the only automated check that the `webfetch:` key survives; nothing else reads it back. |
| `tests/unit/opencode-go-session.test.js` | The third, added in `v0.5.69`, and it **passes**. Its last case constrains fork code by a route the other two do not: it **reads `open-sse/handlers/chatCore.js` as text** and requires exactly two `executor.execute({…})` matches, each naming the provider session and the client tool. | • The count is the fragile half. A third call site added anywhere in that file fails it, and so does a second `})` appearing inside either object literal — the regex is non-greedy and stops at the first one.<br>• It is the only automated check on that file's shape, so it is also the cheapest signal that a conflict resolution there kept upstream's arguments. |
...
| Lint | `npx eslint <changed files>` | • React-hooks purity and set-state rules, which two fork components depend on.<br>• No `lint` script in `package.json`, hence the direct call.<br>• **Scope it to the files you touched:** a repo-wide run reports several hundred pre-existing problems in upstream code, so a total is not a signal.<br>• Scoping does not make every reported problem yours. A merge into a shared file drags upstream's new code into your scope with it — `v0.5.69` put a `set-state-in-effect` error into `profile/page.js` that way. **Attribute before fixing:** write upstream's copy of the file to a non-route name in the same directory, lint that, delete it. The same error from upstream's copy alone means the merge did not cause it, and upstream's code is then left as written. |
| Tests | `npm install --prefix tests`, then `npm test --prefix tests` | • **The suite is its own npm project under `tests/`** — that `package.json` declares the `test` script and pins `vitest`, its config sits beside it, and `tests/README.md` documents the setup. The ROOT `package.json` declares none of it, which is what makes it easy to conclude nothing does.<br>• One upstream test **fails by design** (see the `github-monthly-usage-lock` row above), and the suite carries failures that predate the fork — compare against a baseline, never against zero.<br>• **`tests/__baseline__/` cannot supply that baseline outside a container.** The gate splits each result path on `/app/` to build its key, so off-container every key is `undefined :: …`, nothing matches `known-fails.txt`, and every failure reads as a regression. `known-fails.txt` itself is a container list and is much shorter than a local run's — several suites fail on absolute container paths or native modules that only exist there. **Leave both files alone**; rewriting them to match one machine breaks the gate where it does work.<br>• **The comparison that does work is pre-merge versus post-merge on the same machine.** Run the suite, then run it again from a worktree at the pre-merge commit and diff the failure sets — identical sets is the actual no-regression evidence. The worktree needs the **root** `node_modules` as well as `tests/`, because the vitest config resolves `open-sse` and `@/` to repo source; junction both in rather than reinstalling, and remove a junction with a tool that does not follow it or the delete takes the real tree with it. Put the worktree outside the repo: the config excludes `**/.claude/**`, which would otherwise swallow the copy's own test files.<br>• **A full run writes snapshots**, leaving tracked files under `tests/translator/__snapshots__/` modified and sometimes adding one. On Windows the modification is usually line endings only — an empty `git diff` with the file still listed as changed. Restore them afterwards either way, or the merge commit carries snapshot churn nothing asked for. |
...
|---|---|---|---|
| `v0.5.69` (`eb712ca8`) | 2026-09-06 | `open-sse/handlers/chatCore/streamingHandler.js`, one hunk: the `handleStreamingResponse` destructuring line, where upstream appended `credentials` and the fork had appended `logDir, sessionTag` | Kept both — `+ beside` on both sides. **The merge with the least to resolve so far and the most to check**, because the four files the fork and upstream both touched are the only intersection: this one conflicted and the other three (`chatCore.js`, `src/sse/services/tokenRefresh.js`, `profile/page.js`) auto-merged into fork code and were read by hand. Nothing under `src/sse/handlers/` changed upstream, so [thread 5](#5-the-nine-account-walks)'s blind spot — a one-line fix landing inside the hoisted loop — did not arise this round. One count moved legitimately and was updated in the script: upstream deleted the scheduler's start-up log line, which was one of the `BACKGROUND_REFRESH_LEAD_MS` mentions item 20 counts; all four eligibility guards and the lead formula are intact. Upstream added a **third** test that constrains fork-edited code — see the `opencode-go-session` row. `Dockerfile` was taken verbatim, CN package mirrors included. |
